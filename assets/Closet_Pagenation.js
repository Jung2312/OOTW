...
async function fetchData() {
    try {
        let page = 1;
        const showBtn = 5;
        let url = `https://${ip}:8080/db?page=${page}`;
        ...
        document.querySelectorAll('.type_btn').forEach(btn => {
            btn.addEventListener("click", function (e) {
                    ...
                    url = `https://${ip}:8080/db/category?page=${page}&category=${categoryId}`;
                    fetch(url)
        ...
        const makeBtn = (id) => {
            ...
            btn.addEventListener("click", function (e) {
                ...
                e.target.classList.add("active");
                page = parseInt(e.target.dataset.num);
                url = `https://${ip}:8080/db?page=${page}`;
                fetch(url)
                    .then(response => response.json())
                    .then(data => {
                        displayData(data.clothes);
        ...
        async function deleteSelectedImages() {
            ...
            if(confirmflag){
                if(selectedid != 0){
                    const url = `https://${ip}:8080/clothes_delete/${selectedid}`;
                    try {
                        fetch(url, {
                            method: 'DELETE',
                        });
